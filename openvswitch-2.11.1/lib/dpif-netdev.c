...
#ifdef SIGCOMM_MEASUREMENT
    char name[30];
    sprintf(name, "/rb_%d", core_id == NON_PMD_CORE_ID ? 0 : ++rb_index);
...
// SIGCOMM: write to ringbuffer
        ...
        struct dp_packet **my_packets = packets_->packets;
        struct rte_mbuf * mbuf= &(my_packets[i]->mbuf);

        // VLOG_ERR("receive a packet!");
        if (likely(mbuf != NULL)) {  
            struct ether_hdr *eth_hdr;
            struct ether_addr *ether_hdr_s_addr = NULL;
            struct ether_addr *ether_hdr_d_addr = NULL;
            void* l3_hdr = NULL;
            int l2_len = sizeof(struct ether_hdr);

            eth_hdr = rte_pktmbuf_mtod(mbuf, struct ether_hdr *);
            ether_hdr_s_addr = &eth_hdr->s_addr;
            ether_hdr_d_addr = &eth_hdr->d_addr;

            if(ether_hdr_s_addr != NULL && ether_hdr_d_addr != NULL){
                sprintf(macaddr_src, "%02X:%02X:%02X:%02X:%02X:%02X",
                    ether_hdr_s_addr->addr_bytes[0],
                    ether_hdr_s_addr->addr_bytes[1],
                    ether_hdr_s_addr->addr_bytes[2],
                    ether_hdr_s_addr->addr_bytes[3],
                    ether_hdr_s_addr->addr_bytes[4],
                    ether_hdr_s_addr->addr_bytes[5]);

                sprintf(macaddr_dst, "%02X:%02X:%02X:%02X:%02X:%02X",
                    ether_hdr_d_addr->addr_bytes[0],
                    ether_hdr_d_addr->addr_bytes[1],
                    ether_hdr_d_addr->addr_bytes[2],
                    ether_hdr_d_addr->addr_bytes[3],
                    ether_hdr_d_addr->addr_bytes[4],
                    ether_hdr_d_addr->addr_bytes[5]);

                // VLOG_ERR("Ethernet header size: %d, macsrc: %s, macdst: %s",
                //  l2_len, macaddr_src, macaddr_dst);
            } else{
                VLOG_ERR("Ethernet no mac header");
            }
            ...
            if (eth_hdr->ether_type == rte_cpu_to_be_16(0x0800)) {
                ...
                ip_hdr = (struct ipv4_hdr*)l3_hdr;
                tuple.key.src_ip = ip_hdr->src_addr;
                tuple.key.dst_ip = ip_hdr->dst_addr;

                // VLOG_ERR("IP header src: %08x, dst: %08x", ip_hdr->src_addr, ip_hdr->dst_addr);
